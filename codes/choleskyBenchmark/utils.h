...
{
    // TASK 2.1: Allocate memory on GPU
    cudaError_t err = cudaMalloc((void**)ptr, M * N * sizeof(T));
    if (err != cudaSuccess)
    {
        std::cerr << "CUDA Error: " << cudaGetErrorString(err) << " while allocating GPU memory" << std::endl;
        exit(EXIT_FAILURE);
    }
}
...
{
    // TASK 2.1: Free memory on GPU
    cudaError_t err = cudaFree(ptr);
    if (err != cudaSuccess)
    {
        std::cerr << "CUDA Error: " << cudaGetErrorString(err) << " while freeing GPU memory" << std::endl;
        exit(EXIT_FAILURE);
    }
}
