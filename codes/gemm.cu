...
void tileGemm(cublasHandle_t handle, int M, int N, int K, elem_t alpha,
    ...
    // Loop over all tiles and perform the matrix multiplication
...
int main(int argc, char **argv)
{
	...
	for (int i = 0; i < ITERS + 1; i++) {
		...
		// Use multiple streams to perform the tileGemm in different streams
		for (int streamIndex = 0; streamIndex < NB_STREAMS; ++streamIndex) {
			// Launch the computation using each stream
			tileGemm(handle, M, N, K, alpha, d_A, M, d_B, K, beta, d_C, M, tileM, tileN, streams[streamIndex]);
		}

		// Synchronize all streams to ensure all computations are completed before moving forward
		...
		// Exclude the first warm-up iteration from the average calculation
		if (i > 0) avgTimeGPU += elapsedTime;
	}
