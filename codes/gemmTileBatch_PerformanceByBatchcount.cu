...
int calculateBatchCount(int M, int N, int tileM, int tileN) {
    int batchCount = 0;
...
int main(int argc, char **argv)
{
    ...
    execPerfFile << "M,N,K,tileM,tileN,BatchCount,TimeGPU(ms),PerformanceGPU(GFLOP/s)" << std::endl;

    // Fixed values for M, N, K
    int M = 8000;
    int N = 8000;
    int K = 8000;

    // Different tile sizes for tileM and tileN
    std::vector<int> tileSizes = {32, 64, 128}; // Varying tile sizes
    std::vector<int> nbStreamsOptions = {4, 8, 16}; // Varying number of streams

    for (int tileM : tileSizes) {
        for (int tileN : tileSizes) {
            for (int nb_streams : nbStreamsOptions) {
                // Calculate batch count
                int batchCount = calculateBatchCount(M, N, tileM, tileN);
                ...
                std::cout << "M: " << M << ", N: " << N << ", K: " << K << ", tileM: " << tileM << ", tileN: " << tileN
                          << ", BatchCount: " << batchCount << ", nb_streams: " << nb_streams
                ...
                execPerfFile << M << "," << N << "," << K << "," << tileM << "," << tileN << "," << batchCount << "," << avgTimeGPU << "," << performanceGFLOPsGPU << std::endl;
