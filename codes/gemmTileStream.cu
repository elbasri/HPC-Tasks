...
void tileGemmStreams(cublasHandle_t handle, int M, int N, int K, elem_t alpha,
                     const elem_t* d_A, int lda, const elem_t* d_B, int ldb,
                     elem_t beta, elem_t* d_C, int ldc, int tileM, int tileN,
                     int nb_streams, cudaStream_t *streams) {
    // Loop over all tiles and distribute them across streams
    int streamIdx = 0;
    for (int i = 0; i < M; i += tileM) {
        for (int j = 0; j < N; j += tileN) {
            ...
            CHECK_CUBLAS_ERROR(cublasSetStream(handle, streams[streamIdx]));
            ...
            // Move to the next stream in a round-robin manner
            streamIdx = (streamIdx + 1) % nb_streams;
        }
    }
}
...
int main(int argc, char **argv)
{
    ...
    for (int i = 0; i < ITERS + 1; i++) {
        ...
        // Use the tileGemmStreams function with streams
        tileGemmStreams(handle, M, N, K, alpha, d_A, M, d_B, K, beta, d_C, M, tileM, tileN, NB_STREAMS, streams);
